 ...
 * When reduced motion is active, animations resolve instantly (no movement,
 * no blur, no stagger) while still transitioning opacity for context.
 *
 * Usage:
 *   import { fadeUp, staggerContainer, staggerItem, PANEL_SPRING, prefersReducedMotion } from '../../utils/animations';
...
/** Runtime flag — true when the OS/browser requests reduced motion. */
export const prefersReducedMotion =
  typeof window !== 'undefined' &&
  window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;

/** Instant transition used when reduced motion is active. */
const INSTANT = { duration: 0 };

/** Reduced-motion fade only (no movement, no blur). */
const REDUCED_FADE = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: INSTANT }, exit: { opacity: 0, transition: INSTANT } };

// ─── Spring presets ──────────────────────────────────────────────────────────

/** Standard panel slide spring (sidebar, detail panels, notification panel). */
export const PANEL_SPRING = prefersReducedMotion ? INSTANT : { type: 'spring', stiffness: 320, damping: 32, mass: 0.85 };

/** Modal spring — slightly softer landing. */
export const MODAL_SPRING = prefersReducedMotion ? INSTANT : { type: 'spring', damping: 25, stiffness: 300 };

/** Feed / list spring — gentler for scroll-heavy UIs. */
export const LIST_SPRING = prefersReducedMotion ? INSTANT : { type: 'spring', damping: 26, stiffness: 210 };

/** Bottom sheet spring — snappy. */
export const SHEET_SPRING = prefersReducedMotion ? INSTANT : { type: 'spring', stiffness: 350, damping: 35 };

/** Marker pop spring — extra bouncy. */
export const MARKER_SPRING = prefersReducedMotion ? INSTANT : { type: 'spring', stiffness: 420, damping: 22 };
...
/** Bottom sheet slide. */
export const bottomSheet = prefersReducedMotion
  ? REDUCED_FADE
  : {
      hidden: { y: '100%' },
...
/** Dropdown menu. */
export const dropdown = prefersReducedMotion
  ? REDUCED_FADE
  : {
...
export const markerAppear = prefersReducedMotion
  ? REDUCED_FADE
  : {
...
export const overlayFade = prefersReducedMotion
  ? REDUCED_FADE
  : {
      hidden: { opacity: 0 },
...
export const sectionVariants = prefersReducedMotion
  ? REDUCED_FADE
  : {
...
export const gridVariants = prefersReducedMotion
  ? { hidden: {}, visible: { transition: INSTANT } }
  : {
      hidden: {},
...
export const cardVariants = prefersReducedMotion
  ? REDUCED_FADE
  : {
